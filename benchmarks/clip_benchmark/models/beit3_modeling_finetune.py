...
import clip_benchmark.datasets.beit3_utils as utils
from .beit3_modeling_utils import BEiT3Wrapper, _get_base_config, _get_large_config
    ...
    def forward(
        ...
        **kwargs
        ...
        if image is not None:
            ...
            # skip output head for classification; use head for cross-modal retrieval
            vision_cls = x[:, 0, :]  if not with_head else self.vision_head(x[:, 0, :]) 
        ...
        if text_description is not None:
            ...
            # skip output head for classification; use head for cross-modal retrieval
            language_cls = x[:, 0, :] if not with_head else self.language_head(x[:, 0, :])
...
@register_model
def beit3_base_patch32_224_retrieval(pretrained=False, **kwargs):
    args = _get_base_config(patch_size=32, **kwargs)
    model = BEiT3ForRetrieval(args, **kwargs)
    return model
...
@register_model
def beit3_large_patch32_224_retrieval(pretrained=False, **kwargs):
    args = _get_large_config(img_size=224, patch_size=32, **kwargs)
    model = BEiT3ForRetrieval(args, **kwargs)
    return model
