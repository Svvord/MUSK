...
class CLAM_Batch(nn.Module):
    ...
    def forward(self, batch, return_global_feature=False):
        h, label = batch[0], batch[1]   # h: [b, n, d]
        
        bsz = h.shape[0]
        h = rearrange(h, 'b n d -> (b n) d')
        A, h = self.attention_net(h)  # NxK

        A = rearrange(A, '(b n) d -> b d n', b=bsz) # B x K x N, e.g., 32*1*12000
        h = rearrange(h, '(b n) d -> b n d', b=bsz) # 32*12000*1024
        ...
        if self.instance_eval:
            ...
            for inst in range(bsz):
                ...
                if A_inst.shape[1] < 2 * self.k_sample:
                    self.k_sample = (A_inst.shape[1] - 1) // 2
        ...
        M = torch.bmm(A, h).squeeze(1)  # 32*1024
